    ...
    public class RelayBot : ActivityHandler
    {
        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            using (DirectLineClient client = new DirectLineClient(currentConversation.Token))
            {
                ...
                //     throw new Exception($"Sent {turnContext.Activity.Text } to HAL 9000 with conversation id {currentConversation.ConversationtId} ");
                // //DEBUG
        ...
        private async Task RespondPowerVirtualAgentsBotReplyAsync(DirectLineClient client, RelayConversation currentConversation, ITurnContext<IMessageActivity> turnContext)
        {
            ...
            for (int retry = 0; retry < retryMax; retry++)
            {
                ...
                //     throw new Exception($"DEBUG: Responses: {response.Activities.Count()},  Botname: {_botService.GetBotName()}  ");
                // //END DEBUG
                if (botResponses?.Count() > 0)
                {
                    ...
                    //DEBUG
                    // if(botResponses.Count()>0)
                    //     throw new Exception($"DEBUG: Responses: {botResponses.Count()},  Response: {botResponses.Last().Text}, Topic: {botResponses.Last().TopicName}  ");
                    // // END DEBUG
